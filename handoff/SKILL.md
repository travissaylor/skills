...
3. **Name the file.** From the project root, run `<skill-dir>/scripts/handoff.sh new <slug>`, where the slug names the work in two to four words. It prints a path that does not yet exist. The script names the project from your working directory, so call it by its absolute path and never `cd` into the skill directory to run it. Set `HANDOFF_PROJECT` to name the project yourself when you must run it from elsewhere.
...
5. **Sync.** Run `<skill-dir>/scripts/handoff.sh sync`, again from the project root. It pushes to and pulls from every host in `~/handoffs/peers`. An unreachable peer is reported, not fatal. If no peers are listed, tell the user the store is local only and how to add one.
